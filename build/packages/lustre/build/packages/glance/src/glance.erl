...
-type definition(ETH) :: {definition, list(attribute()), ETH}.
...
-type bit_string_segment_option(ETI) :: bytes_option |
    int_option |
    float_option |
    bits_option |
    utf8_option |
...
-type record_update_field(ETJ) :: {record_update_field,
        binary(),
        gleam@option:option(ETJ)}.

-type variant_field() :: {labelled_variant_field, type(), binary()} |
    {unlabelled_variant_field, type()}.

-type field(ETK) :: {labelled_field, binary(), span(), ETK} |
    {shorthand_field, binary(), span()} |
    {unlabelled_field, ETK}.
...
-type parsed_list(ETL) :: {parsed_list,
        list(ETL),
        gleam@option:option(ETL),
        list({glexer@token:token(), glexer:position()}),
        integer()}.
...
-file("src/glance.gleam", 1745).
-spec list(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {FAB, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    gleam@option:option(fun((span()) -> FAB)),
    list(FAB),
    list({glexer@token:token(), glexer:position()})
) -> {ok, parsed_list(FAB)} | {error, error()}.
...
-file("src/glance.gleam", 2144).
-spec field(
    list({glexer@token:token(), glexer:position()}),
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {FCE, list({glexer@token:token(), glexer:position()})}} |
        {error, error()})
) -> {ok, {field(FCE), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
field(Tokens, Parser) ->
    case Tokens of
...
-file("src/glance.gleam", 1373).
-spec bit_string_segment_options(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {EXY, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    list(bit_string_segment_option(EXY)),
    list({glexer@token:token(), glexer:position()})
) -> {ok,
        {list(bit_string_segment_option(EXY)),
            list({glexer@token:token(), glexer:position()})}} |
...
-file("src/glance.gleam", 1363).
-spec optional_bit_string_segment_options(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {EXR, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    list({glexer@token:token(), glexer:position()})
) -> {ok,
        {list(bit_string_segment_option(EXR)),
            list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-file("src/glance.gleam", 1353).
-spec bit_string_segment(
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {EXK, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    list({glexer@token:token(), glexer:position()})
) -> {ok,
        {{EXK, list(bit_string_segment_option(EXK))},
            list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
-file("src/glance.gleam", 1702).
-spec delimited(
    list(EZR),
    list({glexer@token:token(), glexer:position()}),
    fun((list({glexer@token:token(), glexer:position()})) -> {ok,
            {EZR, list({glexer@token:token(), glexer:position()})}} |
        {error, error()}),
    glexer@token:token()
) -> {ok, {list(EZR), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
...
pattern(Tokens) ->
    gleam@result:'try'(case Tokens of
            ...
            [{{string, V@2}, {position, Start@4}},
                ...
                {{name, L@2}, _},
                {less_greater, _},
                {{discard_name, R@2}, {position, Name_start@3}} |
                Tokens@5] ->
                Span@2 = {span, Start@4, string_offset(Name_start@3, R@2) + 1},
                Pattern@2 = {pattern_concatenate,
                    Span@2,
                    V@2,
                    {some, {named, L@2}},
                    {discarded, R@2}},
            ...
            [{{string, V@3}, {position, Start@5}},
                {less_greater, _},
                {{name, N}, {position, Name_start@4}} |
                Tokens@6] ->
                Span@3 = {span, Start@5, string_offset(Name_start@4, N)},
                Pattern@3 = {pattern_concatenate, Span@3, V@3, none, {named, N}},
                {ok, {Pattern@3, Tokens@6}};

            [{{string, V@4}, {position, Start@6}},
                {less_greater, _},
                {{discard_name, N@1}, {position, Name_start@5}} |
                Tokens@7] ->
                Span@4 = {span, Start@6, string_offset(Name_start@5, N@1) + 1},
                Pattern@4 = {pattern_concatenate,
                    Span@4,
                    V@4,
                    none,
                    {discarded, N@1}},
                {ok, {Pattern@4, Tokens@7}};

            [{{int, Value}, {position, Start@7}} | Tokens@8] ->
                {ok,
                    {{pattern_int, span_from_string(Start@7, Value), Value},
                        Tokens@8}};

            [{{float, Value@1}, {position, Start@8}} | Tokens@9] ->
                {ok,
                    {{pattern_float,
                            span_from_string(Start@8, Value@1),
                            Value@1},
                        Tokens@9}};

            [{{string, Value@2}, {position, Start@9}} | Tokens@10] ->
                {ok,
                    {{pattern_string,
                            {span, Start@9, string_offset(Start@9, Value@2) + 2},
                            Value@2},
                        Tokens@10}};

            [{{discard_name, Name@2}, {position, Start@10}} | Tokens@11] ->
                {ok,
                    {{pattern_discard,
                            {span,
                                Start@10,
                                string_offset(Start@10, Name@2) + 1},
                            Name@2},
                        Tokens@11}};

            [{{name, Name@3}, {position, Start@11}} | Tokens@12] ->
                {ok,
                    {{pattern_variable,
                            span_from_string(Start@11, Name@3),
                            Name@3},
                        Tokens@12}};

            [{left_square, {position, Start@12}} | Tokens@13] ->
                Result = list(
                    fun pattern/1,
                    {some,
                        fun(_capture) ->
                ...
                gleam@result:map(
                    Result,
                    fun(_use0) ->
                        {parsed_list, Elements, Rest, Tokens@14, End} = _use0,
                        {{pattern_list, {span, Start@12, End}, Elements, Rest},
                            Tokens@14}
                    end
                );

            [{hash, {position, Start@13}}, {left_paren, _} | Tokens@15] ->
                ...
                gleam@result:'try'(
                    Result@1,
                    fun(_use0@1) ->
                        {Patterns, End@1, Tokens@16} = _use0@1,
                        {ok,
                            {{pattern_tuple, {span, Start@13, End@1}, Patterns},
                                Tokens@16}}
                    end
                );

            [{less_less, {position, Start@14}} | Tokens@17] ->
                ...
                gleam@result:'try'(
                    Result@2,
                    fun(_use0@2) ->
                        {Segments, End@2, Tokens@18} = _use0@2,
                        {ok,
                            {{pattern_bit_string,
                                    {span, Start@14, End@2},
                                    Segments},
            ...
            {Pattern@5, Tokens@19} = _use0@3,
            case Tokens@19 of
                [{as, _},
                    {{name, Name@4}, {position, Name_start@6}} |
                    Tokens@20] ->
                    Span@5 = {span,
                        erlang:element(2, erlang:element(2, Pattern@5)),
                        string_offset(Name_start@6, Name@4)},
                    Pattern@6 = {pattern_assignment, Span@5, Pattern@5, Name@4},
                    {ok, {Pattern@6, Tokens@20}};
...
-spec expect_name(
    list({glexer@token:token(), glexer:position()}),
    fun((binary(), list({glexer@token:token(), glexer:position()})) -> {ok, EUD} |
        {error, error()})
) -> {ok, EUD} | {error, error()}.
...
-spec until(
    ...
    EUI,
    list({glexer@token:token(), glexer:position()}),
    fun((EUI, list({glexer@token:token(), glexer:position()})) -> {ok,
            {EUI, list({glexer@token:token(), glexer:position()})}} |
        {error, error()})
) -> {ok, {EUI, integer(), list({glexer@token:token(), glexer:position()})}} |
    {error, error()}.
