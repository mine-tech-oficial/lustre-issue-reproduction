...
-type child_specification(ERX) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(ERX)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(ERY)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(ERY).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(ESD)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(ESD).
supervisor(Start) ->
...
-spec significant(child_specification(ESI), boolean()) -> child_specification(ESI).
...
-spec timeout(child_specification(ESL), integer()) -> child_specification(ESL).
timeout(Child, Ms) ->
...
-spec restart(child_specification(ESO), restart()) -> child_specification(ESO).
...
-spec map_data(child_specification(ESR), fun((ESR) -> EST)) -> child_specification(EST).
