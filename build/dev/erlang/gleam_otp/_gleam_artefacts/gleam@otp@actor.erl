...
-type message(DYS) :: {message, DYS} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(DYT, DYU) :: {continue,
        DYT,
        gleam@option:option(gleam@erlang@process:selector(DYU))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(DYV, DYW) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        DYV,
        gleam@erlang@process:selector(message(DYW)),
        gleam@otp@system:debug_state(),
        fun((DYV, DYW) -> next(DYV, DYW))}.

-type started(DYX) :: {started, gleam@erlang@process:pid_(), DYX}.

-opaque initialised(DYY, DYZ, DZA) :: {initialised,
        DYY,
        gleam@option:option(gleam@erlang@process:selector(DYZ)),
        DZA}.

-opaque builder(DZB, DZC, DZD) :: {builder,
        fun((gleam@erlang@process:subject(DZC)) -> {ok,
                initialised(DZB, DZC, DZD)} |
            {error, binary()}),
        integer(),
        fun((DZB, DZC) -> next(DZB, DZC)),
        gleam@option:option(gleam@erlang@process:name(DZC))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(DZE) :: {ack, {ok, DZE} | {error, binary()}} |
...
-spec continue(DZJ) -> next(DZJ, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(DZV, DZW), gleam@erlang@process:selector(DZW)) -> next(DZV, DZW).
...
-spec initialised(EAC) -> initialised(EAC, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(EAH, any(), EAJ),
    gleam@erlang@process:selector(EAN)
) -> initialised(EAH, EAN, EAJ).
...
-spec returning(initialised(EAS, EAT, any()), EAY) -> initialised(EAS, EAT, EAY).
...
-spec new(EBC) -> builder(EBC, EBD, gleam@erlang@process:subject(EBD)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(EBI)) -> {ok, initialised(EBK, EBI, EBL)} |
        {error, binary()})
) -> builder(EBK, EBI, EBL).
...
-spec on_message(builder(EBU, EBV, EBW), fun((EBU, EBV) -> next(EBU, EBV))) -> builder(EBU, EBV, EBW).
...
-spec named(builder(ECF, ECG, ECH), gleam@erlang@process:name(ECG)) -> builder(ECF, ECG, ECH).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(ECU))) -> gleam@erlang@process:selector(message(ECU)).
...
-spec receive_message(self(any(), ECQ)) -> message(ECQ).
...
-spec initialise_actor(
    builder(any(), any(), EDN),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, EDN} | {error, binary()})
...
-spec start(builder(any(), any(), EEA)) -> {ok, started(EEA)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(EEH), EEH) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(EEJ),
    integer(),
    fun((gleam@erlang@process:subject(EEL)) -> EEJ)
) -> EEL.
