...
-opaque supervisor(EUL, EUM) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(EUL, EUM))}.

-type message(EUN, EUO) :: any() | {gleam_phantom, EUN, EUO}.

-opaque builder(EUP, EUQ) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((EUP) -> {ok, gleam@otp@actor:started(EUQ)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(EUP, EUQ)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(EUR, EUS) :: {local,
        gleam@erlang@process:name(message(EUR, EUS))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(EUT) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, EUT}.
...
-type erlang_child_spec_property(EUU, EUV) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((EUU) -> {ok, gleam@otp@actor:started(EUV)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(EUW, EUX))) -> supervisor(EUW, EUX).
get_by_name(Name) ->
...
-spec worker_child(
    fun((EVD) -> {ok, gleam@otp@actor:started(EVE)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(EVD, EVE).
...
-spec supervisor_child(
    fun((EVI) -> {ok, gleam@otp@actor:started(EVJ)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(EVI, EVJ).
...
-spec named(builder(EVN, EVO), gleam@erlang@process:name(message(EVN, EVO))) -> builder(EVN, EVO).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(EVW, EVX), integer(), integer()) -> builder(EVW, EVX).
...
-spec timeout(builder(EWC, EWD), integer()) -> builder(EWC, EWD).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(EWI, EWJ), gleam@otp@supervision:restart()) -> builder(EWI, EWJ).
...
-spec start(builder(EWO, EWP)) -> {ok,
        gleam@otp@actor:started(supervisor(EWO, EWP))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(EXN, EXO)) -> gleam@otp@supervision:child_specification(supervisor(EXN, EXO)).
supervised(Builder) ->
...
-spec start_child(supervisor(EXU, EXV), EXU) -> {ok,
        gleam@otp@actor:started(EXV)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((EYS) -> {ok, gleam@otp@actor:started(EYT)} |
        {error, gleam@otp@actor:start_error()}),
    EYS
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), EYT, gleam@otp@actor:start_error()).
