...
-type valid_message(VFK) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, VFK}.

-type websocket_message(VFL) :: {valid, valid_message(VFL)} | invalid.
...
-type handler_message(VFM) :: {internal, gramps@websocket:frame()} | {user, VFM}.

-type websocket_state(VFN) :: {websocket_state,
        bitstring(),
        VFN,
...
?DOC(false).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(VGR))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(VGR))).
...
-spec apply_frames(
    ...
    fun((VGH, handler_message(VGI), websocket_connection()) -> mist@internal@next:next(VGH, VGI)),
    websocket_connection(),
    mist@internal@next:next(VGH, websocket_message(VGI)),
    fun((VGH) -> nil)
) -> mist@internal@next:next(VGH, websocket_message(VGI)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {VFW,
        gleam@option:option(gleam@erlang@process:selector(VFX))}),
    fun((VFW) -> nil),
    fun((VFW, handler_message(VFX), websocket_connection()) -> mist@internal@next:next(VFW, VFX)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
