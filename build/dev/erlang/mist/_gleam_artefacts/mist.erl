...
-opaque next(VTK, VTL) :: {continue,
        VTK,
        gleam@option:option(gleam@erlang@process:selector(VTL))} |
    normal_stop |
...
-opaque builder(VTM, VTN) :: {builder,
        integer(),
        fun((gleam@http@request:request(VTM)) -> gleam@http@response:response(VTN)),
...
-type websocket_message(VTO) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, VTO}.
...
-type chunk_next(VTP) :: {chunk_continue, VTP} |
    chunk_stop |
    {chunk_abort, binary()}.

-file("src/mist.gleam", 53).
-spec continue(VTQ) -> next(VTQ, any()).
continue(State) ->
    {continue, State, none}.

-file("src/mist.gleam", 57).
-spec with_selector(next(VTU, VTV), gleam@erlang@process:selector(VTV)) -> next(VTU, VTV).
...
-spec convert_next(next(VUJ, VUK)) -> mist@internal@next:next(VUJ, VUK).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(VVL)) -> gleam@http@response:response(VVN))
) -> builder(VVL, VVN).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(VVR, VVS), integer()) -> builder(VVR, VVS).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), VVX),
    integer(),
    gleam@http@response:response(VVX)
) -> builder(mist@internal@http:connection(), VVX).
...
-spec bind(builder(VWJ, VWK), binary()) -> builder(VWJ, VWK).
...
-spec with_ipv6(builder(VWP, VWQ)) -> builder(VWP, VWQ).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(VWV, VWW), binary(), binary()) -> builder(VWV, VWW).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(VXL)
) -> {ok, websocket_message(VXL)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((VXR, websocket_message(VXS), mist@internal@websocket:websocket_connection()) -> next(VXR, VXS)),
    fun((mist@internal@websocket:websocket_connection()) -> {VXR,
        gleam@option:option(gleam@erlang@process:selector(VXS))}),
    fun((VXR) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(VYG)) -> VYI),
    fun((VYI, VYG, s_s_e_connection()) -> gleam@otp@actor:next(VYI, VYG))
) -> gleam@http@response:response(response_data()).
...
-spec chunked(
    gleam@http@request:request(mist@internal@http:connection()),
    gleam@http@response:response(any()),
    fun((gleam@erlang@process:subject(VYR)) -> VYT),
    fun((VYT, VYR, mist@internal@http:connection()) -> chunk_next(VYT))
) -> gleam@http@response:response(response_data()).
...
-spec chunk_continue(VYY) -> chunk_next(VYY).
