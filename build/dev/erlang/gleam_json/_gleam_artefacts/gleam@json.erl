...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(DWK)) -> {ok, DWK} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(DWE)) -> {ok, DWE} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(DWA)) -> {ok, DWA} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(DWQ), fun((DWQ) -> json())) -> json().
...
-spec array(list(DWU), fun((DWU) -> json())) -> json().
