...
-spec run(gleam@dynamic:dynamic_(), decoder(BWY)) -> {ok, BWY} |
...
-spec run_dynamic_function(
    ...
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, BYT} | {error, BYT})
...
-spec map(decoder(CAW), fun((CAW) -> CAY)) -> decoder(CAY).
...
-spec one_of(decoder(CBN), list(decoder(CBN))) -> decoder(CBN).
...
-spec push_path({BXU, list(decode_error())}, list(any())) -> {BXU,
    list(decode_error())}.
...
-spec list(decoder(BZL)) -> decoder(list(BZL)).
list(Inner) ->
    {decoder,
        fun(Data) ->
            gleam_stdlib:list(
...
-spec index(
    list(BXI),
    list(BXI),
    fun((gleam@dynamic:dynamic_()) -> {BXL, list(decode_error())}),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_(), list(BXI)) -> {BXL, list(decode_error())})
) -> {BXL, list(decode_error())}.
...
-spec subfield(list(any()), decoder(BWT), fun((BWT) -> decoder(BWV))) -> decoder(BWV).
...
-spec at(list(any()), decoder(BXF)) -> decoder(BXF).
at(Path, Inner) ->
    {decoder,
        fun(Data) ->
            index(
...
-spec field(any(), decoder(BYD), fun((BYD) -> decoder(BYF))) -> decoder(BYF).
...
-spec optional_field(any(), BYJ, decoder(BYJ), fun((BYJ) -> decoder(BYL))) -> decoder(BYL).
...
-spec optionally_at(list(any()), BYQ, decoder(BYQ)) -> decoder(BYQ).
...
-spec fold_dict(
    {gleam@dict:dict(CAE, CAF), list(decode_error())},
    gleam@dynamic:dynamic_(),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_()) -> {CAE, list(decode_error())}),
    fun((gleam@dynamic:dynamic_()) -> {CAF, list(decode_error())})
) -> {gleam@dict:dict(CAE, CAF), list(decode_error())}.
...
-spec dict(decoder(BZX), decoder(BZZ)) -> decoder(gleam@dict:dict(BZX, BZZ)).
dict(Key, Value) ->
...
-spec optional(decoder(CAS)) -> decoder(gleam@option:option(CAS)).
optional(Inner) ->
...
-spec collapse_errors(decoder(CBF), binary()) -> decoder(CBF).
...
-spec then(decoder(CBI), fun((CBI) -> decoder(CBK))) -> decoder(CBK).
then(Decoder, Next) ->
    {decoder,
        fun(Dynamic_data) ->
...
-spec failure(CBX, binary()) -> decoder(CBX).
...
-spec new_primitive_decoder(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, CBZ} | {error, CBZ})
) -> decoder(CBZ).
...
-spec recursive(fun(() -> decoder(CCD))) -> decoder(CCD).
recursive(Inner) ->
    {decoder,
        fun(Data) ->
            Decoder = Inner(),
