...
-opaque set(CWN) :: {set, gleam@dict:dict(CWN, list(nil))}.
...
-spec insert(set(CWV), CWV) -> set(CWV).
insert(Set, Member) ->
...
-spec contains(set(CWY), CWY) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(CXA), CXA) -> set(CXA).
delete(Set, Member) ->
...
-spec from_list(list(CXG)) -> set(CXG).
from_list(Members) ->
...
-spec fold(set(CXJ), CXL, fun((CXL, CXJ) -> CXL)) -> CXL.
...
-spec filter(set(CXM), fun((CXM) -> boolean())) -> set(CXM).
...
-spec map(set(CXP), fun((CXP) -> CXR)) -> set(CXR).
map(Set, Fun) ->
...
-spec drop(set(CXT), list(CXT)) -> set(CXT).
drop(Set, Disallowed) ->
...
-spec take(set(CXX), list(CXX)) -> set(CXX).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 290).
-spec order(set(CYF), set(CYF)) -> {set(CYF), set(CYF)}.
order(First, Second) ->
...
-spec union(set(CYB), set(CYB)) -> set(CYB).
union(First, Second) ->
...
-spec intersection(set(CYK), set(CYK)) -> set(CYK).
...
-spec difference(set(CYO), set(CYO)) -> set(CYO).
...
-spec is_subset(set(CYS), set(CYS)) -> boolean().
...
-spec is_disjoint(set(CYV), set(CYV)) -> boolean().
...
-spec symmetric_difference(set(CYY), set(CYY)) -> set(CYY).
...
-spec each(set(CZC), fun((CZC) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
