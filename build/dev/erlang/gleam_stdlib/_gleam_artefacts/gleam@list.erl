...
-spec repeat_loop(AMK, integer(), list(AMK)) -> list(AMK).
...
-spec repeat(AMI, integer()) -> list(AMI).
repeat(A, Times) ->
    repeat_loop(A, Times, []).

-file("src/gleam/list.gleam", 1435).
-spec split_loop(list(AMR), integer(), list(AMR)) -> {list(AMR), list(AMR)}.
...
-spec split(list(AMN), integer()) -> {list(AMN), list(AMN)}.
split(List, Index) ->
    split_loop(List, Index, []).

-file("src/gleam/list.gleam", 1471).
-spec split_while_loop(list(ANA), fun((ANA) -> boolean()), list(ANA)) -> {list(ANA),
    list(ANA)}.
...
-spec split_while(list(AMW), fun((AMW) -> boolean())) -> {list(AMW), list(AMW)}.
...
-spec key_find(list({ANF, ANG}), ANF) -> {ok, ANG} | {error, nil}.
...
-spec key_filter(list({ANK, ANL}), ANK) -> list(ANL).
...
-spec key_pop_loop(list({ANU, ANV}), ANU, list({ANU, ANV})) -> {ok,
        {ANV, list({ANU, ANV})}} |
    {error, nil}.
...
-spec key_set_loop(list({AOF, AOG}), AOF, AOG, list({AOF, AOG})) -> list({AOF,
    AOG}).
...
-spec partition_loop(list(BHL), fun((BHL) -> boolean()), list(BHL), list(BHL)) -> {list(BHL),
    list(BHL)}.
...
-spec permutation_zip(list(API), list(API), list(list(API))) -> list(list(API)).
...
-spec window_loop(list(list(AQC)), list(AQC), integer()) -> list(list(AQC)).
...
-spec window(list(APY), integer()) -> list(list(APY)).
window(List, N) ->
    case N =< 0 of
        true ->
            [];
...
-spec window_by_2(list(AQI)) -> list({AQI, AQI}).
window_by_2(List) ->
...
-spec drop_while(list(AQL), fun((AQL) -> boolean())) -> list(AQL).
...
-spec take_while_loop(list(AQR), fun((AQR) -> boolean()), list(AQR)) -> list(AQR).
...
-spec take_while(list(AQO), fun((AQO) -> boolean())) -> list(AQO).
take_while(List, Predicate) ->
    take_while_loop(List, Predicate, []).

-file("src/gleam/list.gleam", 1863).
-spec chunk_loop(list(ARA), fun((ARA) -> ARC), ARC, list(ARA), list(list(ARA))) -> list(list(ARA)).
...
-spec chunk(list(AQV), fun((AQV) -> any())) -> list(list(AQV)).
chunk(List, F) ->
    case List of
        [] ->
            [];
...
-spec sized_chunk(list(ARI), integer()) -> list(list(ARI)).
...
-spec reduce(list(ART), fun((ART, ART) -> ART)) -> {ok, ART} | {error, nil}.
reduce(List, Fun) ->
    case List of
        [] ->
            {error, nil};
...
-spec scan_loop(list(ASB), ASD, list(ASD), fun((ASD, ASB) -> ASD)) -> list(ASD).
...
-spec scan(list(ARX), ARZ, fun((ARZ, ARX) -> ARZ)) -> list(ARZ).
...
-spec last(list(ASG)) -> {ok, ASG} | {error, nil}.
last(List) ->
    case List of
        [] ->
            {error, nil};
...
-spec combinations(list(ASK), integer()) -> list(list(ASK)).
...
-spec combination_pairs_loop(list(ASR), list({ASR, ASR})) -> list({ASR, ASR}).
...
-spec combination_pairs(list(ASO)) -> list({ASO, ASO}).
combination_pairs(Items) ->
    combination_pairs_loop(Items, []).

-file("src/gleam/list.gleam", 2107).
-spec take_firsts(list(list(ATL)), list(ATL), list(list(ATL))) -> {list(ATL),
    list(list(ATL))}.
...
-spec transpose_loop(list(list(ATE)), list(list(ATE))) -> list(list(ATE)).
...
-spec transpose(list(list(ASZ))) -> list(list(ASZ)).
...
-spec interleave(list(list(ASV))) -> list(ASV).
interleave(List) ->
    _pipe = List,
...
-spec shuffle_pair_unwrap_loop(list({float(), ATX}), list(ATX)) -> list(ATX).
...
-spec do_shuffle_by_pair_indexes(list({float(), AUB})) -> list({float(), AUB}).
...
-spec shuffle(list(ATU)) -> list(ATU).
shuffle(List) ->
    _pipe = List,
...
-spec max_loop(list(AUL), fun((AUL, AUL) -> gleam@order:order()), AUL) -> AUL.
...
-spec max(list(AUE), fun((AUE, AUE) -> gleam@order:order())) -> {ok, AUE} |
    {error, nil}.
max(List, Compare) ->
    case List of
        [] ->
...
-spec build_reservoir_loop(
    list(AVA),
    integer(),
    gleam@dict:dict(integer(), AVA)
) -> {gleam@dict:dict(integer(), AVA), list(AVA)}.
...
-spec build_reservoir(list(AUV), integer()) -> {gleam@dict:dict(integer(), AUV),
    list(AUV)}.
...
-spec sample(list(AUM), integer()) -> list(AUM).
sample(List, N) ->
