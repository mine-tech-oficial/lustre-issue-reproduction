...
-type state(KDH, KDI) :: {state,
        gleam@erlang@process:subject(message(KDI)),
        gleam@erlang@process:selector(message(KDI)),
        gleam@erlang@process:selector(message(KDI)),
        KDH,
        fun((KDH, KDI) -> {KDH, lustre@effect:effect(KDI)}),
        fun((KDH) -> lustre@vdom@vnode:element(KDI)),
        config(KDI),
        lustre@vdom@vnode:element(KDI),
        lustre@vdom@cache:cache(KDI),
        gleam@dict:dict(binary(), gleam@json:json()),
        gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(KDI)), gleam@erlang@process:monitor()),
        gleam@set:set(fun((lustre@runtime@transport:client_message(KDI)) -> nil))}.

-type config(KDJ) :: {config,
        boolean(),
        boolean(),
        gleam@dict:dict(binary(), fun((binary()) -> {ok, KDJ} | {error, nil})),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(KDJ)),
        gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(KDJ)),
        gleam@option:option(KDJ),
        gleam@option:option(KDJ)}.

-type message(KDK) :: {client_dispatched_message,
        lustre@runtime@transport:server_message()} |
    {client_registered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(KDK))} |
    {client_deregistered_subject,
        gleam@erlang@process:subject(lustre@runtime@transport:client_message(KDK))} |
    {client_registered_callback,
        fun((lustre@runtime@transport:client_message(KDK)) -> nil)} |
    {client_deregistered_callback,
        fun((lustre@runtime@transport:client_message(KDK)) -> nil)} |
    {effect_added_selector, gleam@erlang@process:selector(message(KDK))} |
    {effect_dispatched_message, KDK} |
    ...
    {effect_requested_context_subscription,
        binary(),
        gleam@dynamic@decode:decoder(KDK)} |
...
?DOC(false).
-spec broadcast(
    gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(KFK)), gleam@erlang@process:monitor()),
    gleam@set:set(fun((lustre@runtime@transport:client_message(KFK)) -> nil)),
    lustre@runtime@transport:client_message(KFK)
) -> nil.
...
?DOC(false).
-spec handle_effect(
    gleam@erlang@process:subject(message(KFG)),
    lustre@effect:effect(KFG)
) -> nil.
...
-spec handle_property_change(
    gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(KFA)),
    binary(),
...
-spec handle_attribute_change(
    gleam@dict:dict(binary(), fun((binary()) -> {ok, KET} | {error, nil})),
    binary(),
    binary()
) -> {ok, KET} | {error, nil}.
...
?DOC(false).
-spec loop(state(KED, KEE), message(KEE)) -> gleam@otp@actor:next(state(KED, KEE), message(KEE)).
loop(State, Message) ->
    case Message of
...
?DOC(false).
-spec start(
    gleam@option:option(gleam@erlang@process:name(message(KDO))),
    fun((KDS) -> {KDT, lustre@effect:effect(KDO)}),
    fun((KDT, KDO) -> {KDT, lustre@effect:effect(KDO)}),
    fun((KDT) -> lustre@vdom@vnode:element(KDO)),
    config(KDO),
    KDS
) -> {ok, gleam@otp@actor:started(gleam@erlang@process:subject(message(KDO)))} |
