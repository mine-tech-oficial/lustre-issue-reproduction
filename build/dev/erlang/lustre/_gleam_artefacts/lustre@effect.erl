...
-opaque effect(GKQ) :: {effect,
        list(fun((actions(GKQ)) -> nil)),
        list(fun((actions(GKQ)) -> nil)),
        list(fun((actions(GKQ)) -> nil))}.

-type actions(GKR) :: {actions,
        fun((GKR) -> nil),
        fun((binary(), gleam@json:json()) -> nil),
        fun((gleam@erlang@process:selector(GKR)) -> nil),
        fun(() -> gleam@dynamic:dynamic_()),
        fun((binary(), gleam@json:json()) -> nil),
        fun((binary(), gleam@dynamic@decode:decoder(GKR)) -> nil),
        fun((binary()) -> nil)}.
...
-spec from(fun((fun((GKU) -> nil)) -> nil)) -> effect(GKU).
from(Effect) ->
    Task = fun(Actions) ->
...
-spec before_paint(fun((fun((GKW) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(GKW).
before_paint(Effect) ->
    Task = fun(Actions) ->
...
-spec after_paint(fun((fun((GKY) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(GKY).
after_paint(Effect) ->
    Task = fun(Actions) ->
        Root = (erlang:element(5, Actions))(),
        Dispatch = erlang:element(2, Actions),
...
?DOC(false).
-spec select(
    fun((fun((GLC) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(GLC))
) -> effect(GLC).
select(Sel) ->
    Task = fun(Actions) ->
...
-spec subscribe(binary(), gleam@dynamic@decode:decoder(GLJ)) -> effect(GLJ).
...
-spec batch(list(effect(GLO))) -> effect(GLO).
batch(Effects) ->
    gleam@list:fold(
        Effects,
        {effect, [], [], []},
...
-spec do_comap_select(
    actions(GMG),
    gleam@erlang@process:selector(GMI),
    fun((GMI) -> GMG)
) -> nil.
...
-spec do_comap_actions(actions(GMC), fun((GME) -> GMC)) -> actions(GME).
...
-spec do_map(list(fun((actions(GLW)) -> nil)), fun((GLW) -> GLZ)) -> list(fun((actions(GLZ)) -> nil)).
do_map(Effects, F) ->
    gleam@list:map(
        Effects,
        fun(Effect) ->
...
-spec map(effect(GLS), fun((GLS) -> GLU)) -> effect(GLU).
map(Effect, F) ->
    {effect,
...
-spec perform(
    ...
    fun((GMK) -> nil),
    fun((binary(), gleam@json:json()) -> nil),
    fun((gleam@erlang@process:selector(GMK)) -> nil),
    fun(() -> gleam@dynamic:dynamic_()),
    fun((binary(), gleam@json:json()) -> nil),
    fun((binary(), gleam@dynamic@decode:decoder(GMK)) -> nil),
    fun((binary()) -> nil)
) -> nil.
