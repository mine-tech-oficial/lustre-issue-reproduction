...
-spec do_extract_keyed_children(
    list({binary(), lustre@vdom@vnode:element(NRK)}),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(NRK)),
    list(lustre@vdom@vnode:element(NRK))
) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(NRK)),
    list(lustre@vdom@vnode:element(NRK))}.
...
-spec extract_keyed_children(list({binary(), lustre@vdom@vnode:element(NRC)})) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(NRC)),
    list(lustre@vdom@vnode:element(NRC))}.
...
-spec element(
    binary(),
    list(lustre@vdom@vattr:attribute(NPI)),
    list({binary(), lustre@vdom@vnode:element(NPI)})
) -> lustre@vdom@vnode:element(NPI).
...
-spec namespaced(
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(NPO)),
    list({binary(), lustre@vdom@vnode:element(NPO)})
) -> lustre@vdom@vnode:element(NPO).
...
-spec fragment(list({binary(), lustre@vdom@vnode:element(NPU)})) -> lustre@vdom@vnode:element(NPU).
fragment(Children) ->
    {Keyed_children, Children@1} = extract_keyed_children(Children),
...
-spec ul(
    list(lustre@vdom@vattr:attribute(NPY)),
    list({binary(), lustre@vdom@vnode:element(NPY)})
) -> lustre@vdom@vnode:element(NPY).
ul(Attributes, Children) ->
    element(<<"ul"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 145).
-spec ol(
    list(lustre@vdom@vattr:attribute(NQE)),
    list({binary(), lustre@vdom@vnode:element(NQE)})
) -> lustre@vdom@vnode:element(NQE).
ol(Attributes, Children) ->
    element(<<"ol"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 152).
-spec 'div'(
    list(lustre@vdom@vattr:attribute(NQK)),
    list({binary(), lustre@vdom@vnode:element(NQK)})
) -> lustre@vdom@vnode:element(NQK).
'div'(Attributes, Children) ->
    element(<<"div"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 159).
-spec tbody(
    list(lustre@vdom@vattr:attribute(NQQ)),
    list({binary(), lustre@vdom@vnode:element(NQQ)})
) -> lustre@vdom@vnode:element(NQQ).
tbody(Attributes, Children) ->
    element(<<"tbody"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 166).
-spec dl(
    list(lustre@vdom@vattr:attribute(NQW)),
    list({binary(), lustre@vdom@vnode:element(NQW)})
) -> lustre@vdom@vnode:element(NQW).
