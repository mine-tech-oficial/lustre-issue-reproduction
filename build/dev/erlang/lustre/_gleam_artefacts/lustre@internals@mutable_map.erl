...
-type mutable_map(GSC, GSD) :: any() | {gleam_phantom, GSC, GSD}.
...
?DOC(false).
-spec unsafe_get(mutable_map(GSI, GSJ), GSI) -> GSJ.
unsafe_get(Map, Key) ->
    maps:get(Key, Map).

-file("src/lustre/internals/mutable_map.gleam", 34).
?DOC(false).
-spec get_or_compute(mutable_map(GSQ, GSR), GSQ, fun(() -> GSR)) -> GSR.
...
?DOC(false).
-spec has_key(mutable_map(GTA, any()), GTA) -> boolean().
has_key(Map, Key) ->
    maps:is_key(Key, Map).

-file("src/lustre/internals/mutable_map.gleam", 61).
?DOC(false).
-spec insert(mutable_map(GTI, GTJ), GTI, GTJ) -> mutable_map(GTI, GTJ).
insert(Map, Key, Value) ->
    maps:put(Key, Value, Map).

-file("src/lustre/internals/mutable_map.gleam", 79).
?DOC(false).
-spec delete(mutable_map(GTU, GTV), GTU) -> mutable_map(GTU, GTV).
delete(Map, Key) ->
    maps:remove(Key, Map).
