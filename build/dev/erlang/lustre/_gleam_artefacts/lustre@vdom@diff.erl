...
-type diff(JRN) :: {diff,
        lustre@vdom@patch:patch(JRN),
        lustre@vdom@cache:cache(JRN)}.

-type partial_diff(JRO) :: {partial_diff,
        lustre@vdom@patch:patch(JRO),
        lustre@vdom@cache:cache(JRO),
        lustre@vdom@cache:events(JRO)}.

-type attribute_change(JRP) :: {attribute_change,
        list(lustre@vdom@vattr:attribute(JRP)),
        list(lustre@vdom@vattr:attribute(JRP)),
        lustre@vdom@cache:events(JRP)}.
...
-spec diff_attributes(
    ...
    lustre@vdom@cache:events(JSS),
    list(lustre@vdom@vattr:attribute(JSS)),
    list(lustre@vdom@vattr:attribute(JSS)),
    list(lustre@vdom@vattr:attribute(JSS)),
    list(lustre@vdom@vattr:attribute(JSS))
) -> attribute_change(JSS).
...
?DOC(false).
-spec do_diff(
    list(lustre@vdom@vnode:element(JRV)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(JRV)),
    list(lustre@vdom@vnode:element(JRV)),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(JRV)),
    ...
    integer(),
    integer(),
    list(lustre@vdom@patch:change(JRV)),
    list(lustre@vdom@patch:patch(JRV)),
    lustre@vdom@path:path(),
    lustre@vdom@cache:cache(JRV),
    lustre@vdom@cache:events(JRV)
) -> partial_diff(JRV).
do_diff(
    Old,
    Old_keyed,
