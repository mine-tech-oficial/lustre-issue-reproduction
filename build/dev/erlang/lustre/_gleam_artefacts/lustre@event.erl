...
-spec on(binary(), gleam@dynamic@decode:decoder(OQJ)) -> lustre@vdom@vattr:attribute(OQJ).
on(Name, Handler) ->
...
-spec advanced(
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(OQM))
) -> lustre@vdom@vattr:attribute(OQM).
...
-spec handler(OQQ, boolean(), boolean()) -> lustre@vdom@vattr:handler(OQQ).
...
-spec prevent_default(lustre@vdom@vattr:attribute(OQS)) -> lustre@vdom@vattr:attribute(OQS).
...
-spec stop_propagation(lustre@vdom@vattr:attribute(OQV)) -> lustre@vdom@vattr:attribute(OQV).
...
-spec debounce(lustre@vdom@vattr:attribute(OQY), integer()) -> lustre@vdom@vattr:attribute(OQY).
...
-spec throttle(lustre@vdom@vattr:attribute(ORB), integer()) -> lustre@vdom@vattr:attribute(ORB).
...
-spec on_keypress(fun((binary()) -> ORS)) -> lustre@vdom@vattr:attribute(ORS).
on_keypress(Message) ->
    on(
        <<"keypress"/utf8>>,
        begin
...
-spec on_keydown(fun((binary()) -> ORU)) -> lustre@vdom@vattr:attribute(ORU).
on_keydown(Message) ->
    on(
        <<"keydown"/utf8>>,
        begin
...
-spec on_keyup(fun((binary()) -> ORW)) -> lustre@vdom@vattr:attribute(ORW).
on_keyup(Message) ->
    on(
        <<"keyup"/utf8>>,
        begin
...
-spec on_input(fun((binary()) -> ORY)) -> lustre@vdom@vattr:attribute(ORY).
on_input(Message) ->
    on(
        <<"input"/utf8>>,
        begin
...
-spec on_change(fun((binary()) -> OSA)) -> lustre@vdom@vattr:attribute(OSA).
on_change(Message) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_check(fun((boolean()) -> OSC)) -> lustre@vdom@vattr:attribute(OSC).
on_check(Message) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_submit(fun((list({binary(), binary()})) -> OSF)) -> lustre@vdom@vattr:attribute(OSF).
on_submit(Message) ->
    _pipe@2 = on(
        <<"submit"/utf8>>,
        begin
...
-spec on_focus(OSJ) -> lustre@vdom@vattr:attribute(OSJ).
on_focus(Message) ->
    on(<<"focus"/utf8>>, gleam@dynamic@decode:success(Message)).

-file("src/lustre/event.gleam", 357).
-spec on_blur(OSL) -> lustre@vdom@vattr:attribute(OSL).
on_blur(Message) ->
