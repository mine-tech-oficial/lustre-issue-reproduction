...
-type attribute(FLF) :: {attribute, integer(), binary(), binary()} |
    {property, integer(), binary(), gleam@json:json()} |
    {event,
        integer(),
        binary(),
        gleam@dynamic@decode:decoder(handler(FLF)),
        list(binary()),
        event_behaviour(),
        event_behaviour(),
        integer(),
...
-type handler(FLG) :: {handler, boolean(), boolean(), FLG}.
...
-spec event(
    binary(),
    gleam@dynamic@decode:decoder(handler(FLL)),
    list(binary()),
    event_behaviour(),
    event_behaviour(),
    integer(),
...
?DOC(false).
-spec merge(list(attribute(FLV)), list(attribute(FLV))) -> list(attribute(FLV)).
...
?DOC(false).
-spec compare(attribute(FMC), attribute(FMC)) -> gleam@order:order().
compare(A, B) ->
    gleam@string:compare(erlang:element(3, A), erlang:element(3, B)).

-file("src/lustre/vdom/vattr.gleam", 91).
?DOC(false).
-spec prepare(list(attribute(FLQ))) -> list(attribute(FLQ)).
prepare(Attributes) ->
    case Attributes of
        [] ->
            Attributes;
