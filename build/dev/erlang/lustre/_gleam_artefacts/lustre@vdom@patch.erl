...
-type change(IPK) :: {replace_text, integer(), binary()} |
    ...
    {replace, integer(), integer(), lustre@vdom@vnode:element(IPK)} |
    {remove, integer(), integer()} |
    {insert, integer(), list(lustre@vdom@vnode:element(IPK)), integer()}.

-file("src/lustre/vdom/patch.gleam", 77).
?DOC(false).
-spec new(integer(), integer(), list(change(IPL)), list(patch(IPL))) -> patch(IPL).
...
?DOC(false).
-spec update(
    list(lustre@vdom@vattr:attribute(IPV)),
    list(lustre@vdom@vattr:attribute(IPV))
) -> change(IPV).
...
?DOC(false).
-spec replace(integer(), lustre@vdom@vnode:element(IQF)) -> change(IQF).
replace(Index, With) ->
    {replace, 5, Index, With}.

-file("src/lustre/vdom/patch.gleam", 127).
?DOC(false).
-spec insert(list(lustre@vdom@vnode:element(IQI)), integer()) -> change(IQI).
...
?DOC(false).
-spec add_parent(patch(IQO), integer()) -> patch(IQO).
...
-spec insert_to_json(
    integer(),
    list(lustre@vdom@vnode:element(ITF)),
    integer(),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(ITF)), lustre@vdom@vnode:element(ITF))
) -> gleam@json:json().
...
-spec replace_to_json(
    ...
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(ITH)), lustre@vdom@vnode:element(ITH))
) -> gleam@json:json().
...
-spec change_to_json(
    change(IQU),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(IQU)), lustre@vdom@vnode:element(IQU))
) -> gleam@json:json().
...
-spec to_json(
    patch(IQR),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(IQR)), lustre@vdom@vnode:element(IQR))
) -> gleam@json:json().
to_json(Patch, Memos) ->
