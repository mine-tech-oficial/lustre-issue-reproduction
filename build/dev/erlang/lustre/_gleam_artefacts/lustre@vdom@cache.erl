...
-opaque cache(JAW) :: {cache,
        events(JAW),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(JAW)), lustre@vdom@vnode:element(JAW)),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(JAW)), lustre@vdom@vnode:element(JAW)),
        list(binary()),
        list(binary())}.

-opaque events(JAX) :: {events,
        lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(JAX))),
        lustre@internals@mutable_map:mutable_map(binary(), child(JAX))}.

-type child(JAY) :: {child,
        fun((gleam@dynamic:dynamic_()) -> gleam@dynamic:dynamic_()),
        events(JAY)}.

-type added_children(JAZ) :: {added_children,
        lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(JAZ))),
        lustre@internals@mutable_map:mutable_map(binary(), child(JAZ)),
        lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(JAZ)), lustre@vdom@vnode:element(JAZ))}.

-opaque decoded_event(JBA) :: {decoded_event,
        binary(),
        lustre@vdom@vattr:handler(JBA)} |
...
?DOC(false).
-spec do_add_event(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(JCW))),
    lustre@vdom@path:path(),
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(JCW))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(JCW))).
...
?DOC(false).
-spec add_attributes(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(JDZ))),
    lustre@vdom@path:path(),
...
-spec do_add_children(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(JER))),
    lustre@internals@mutable_map:mutable_map(binary(), child(JER)),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(JER)), lustre@vdom@vnode:element(JER)),
    lustre@vdom@path:path(),
    integer(),
    list(lustre@vdom@vnode:element(JER))
) -> added_children(JER).
...
?DOC(false).
-spec from_node(lustre@vdom@vnode:element(JBF)) -> cache(JBF).
from_node(Root) ->
    Cache = new(),
...
?DOC(false).
-spec events(cache(JBL)) -> events(JBL).
events(Cache) ->
    erlang:element(2, Cache).

-file("src/lustre/vdom/cache.gleam", 106).
?DOC(false).
-spec update_events(cache(JBO), events(JBO)) -> cache(JBO).
...
?DOC(false).
-spec memos(cache(JBS)) -> lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(JBS)), lustre@vdom@vnode:element(JBS)).
memos(Cache) ->
    erlang:element(3, Cache).
...
-spec get_old_memo(
    cache(JBV),
    fun(() -> lustre@vdom@vnode:element(JBV)),
    fun(() -> lustre@vdom@vnode:element(JBV))
) -> lustre@vdom@vnode:element(JBV).
...
-spec keep_memo(
    cache(JCA),
    fun(() -> lustre@vdom@vnode:element(JCA)),
    fun(() -> lustre@vdom@vnode:element(JCA))
) -> cache(JCA).
...
-spec add_memo(
    cache(JCF),
    fun(() -> lustre@vdom@vnode:element(JCF)),
    lustre@vdom@vnode:element(JCF)
) -> cache(JCF).
...
-spec add_event(
    ...
    lustre@vdom@path:path(),
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(JCR))
) -> events(JCR).
...
-spec do_remove_event(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(JDK))),
    lustre@vdom@path:path(),
    binary()
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(JDK))).
...
?DOC(false).
-spec remove_event(events(JDH), lustre@vdom@path:path(), binary()) -> events(JDH).
...
-spec remove_attributes(
    ...
    lustre@vdom@path:path(),
    list(lustre@vdom@vattr:attribute(JFI))
) -> lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(JFI))).
...
-spec do_remove_children(
    lustre@internals@mutable_map:mutable_map(binary(), gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(JFT))),
    lustre@internals@mutable_map:mutable_map(binary(), child(JFT)),
    lustre@internals@mutable_map:mutable_map(fun(() -> lustre@vdom@vnode:element(JFT)), lustre@vdom@vnode:element(JFT)),
    lustre@vdom@path:path(),
    integer(),
    list(lustre@vdom@vnode:element(JFT))
) -> events(JFT).
...
-spec replace_child(
    ...
    lustre@vdom@path:path(),
    integer(),
    lustre@vdom@vnode:element(JGF),
    lustre@vdom@vnode:element(JGF)
) -> {cache(JGF), events(JGF)}.
...
?DOC(false).
-spec dispatch(cache(JGT), decoded_event(JGT)) -> {cache(JGT),
    {ok, lustre@vdom@vattr:handler(JGT)} | {error, nil}}.
...
?DOC(false).
-spec handle(cache(JGX), binary(), binary(), gleam@dynamic:dynamic_()) -> {cache(JGX),
    {ok, lustre@vdom@vattr:handler(JGX)} | {error, nil}}.
