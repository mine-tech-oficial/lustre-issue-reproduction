...
-type element(GVK) :: {fragment,
        integer(),
        binary(),
        list(element(GVK)),
        lustre@internals@mutable_map:mutable_map(binary(), element(GVK))} |
    {element,
        integer(),
        binary(),
        binary(),
        binary(),
        list(lustre@vdom@vattr:attribute(GVK)),
        list(element(GVK)),
        lustre@internals@mutable_map:mutable_map(binary(), element(GVK)),
        boolean(),
        boolean()} |
...
-spec fragment(
    binary(),
    list(element(GVS)),
    lustre@internals@mutable_map:mutable_map(binary(), element(GVS))
) -> element(GVS).
...
-spec element(
    ...
    list(element(GVZ)),
    lustre@internals@mutable_map:mutable_map(binary(), element(GVZ)),
    boolean(),
    boolean()
) -> element(GVZ).
element(
...
-spec unsafe_inner_html(
    ...
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(GWK)),
    binary()
) -> element(GWK).
...
?DOC(false).
-spec map(element(GWO), fun((GWO) -> GWQ)) -> element(GWQ).
map(Element, Mapper) ->
    case Element of
...
?DOC(false).
-spec memo(binary(), list(lustre@internals@ref:ref()), fun(() -> element(GWT))) -> element(GWT).
memo(Key, Dependencies, View) ->
    {memo, 5, Key, Dependencies, View}.

-file("src/lustre/vdom/vnode.gleam", 201).
?DOC(false).
-spec to_keyed(binary(), element(GWY)) -> element(GWY).
to_keyed(Key, Node) ->
    case Node of
...
-spec memo_to_json(
    fun(() -> element(GXB)),
    lustre@internals@mutable_map:mutable_map(fun(() -> element(GXB)), element(GXB))
) -> gleam@json:json().
...
-spec element_to_json(
    ...
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(GXB)),
    list(element(GXB)),
    lustre@internals@mutable_map:mutable_map(fun(() -> element(GXB)), element(GXB))
) -> gleam@json:json().
