...
-type runtime(MHW) :: any() | {gleam_phantom, MHW}.
...
-spec application(
    fun((MIS) -> {MIT, lustre@effect:effect(MIU)}),
    fun((MIT, MIU) -> {MIT, lustre@effect:effect(MIU)}),
    fun((MIT) -> lustre@vdom@vnode:element(MIU))
) -> lustre@runtime@app:app(MIS, MIT, MIU).
...
-spec element(lustre@vdom@vnode:element(MIF)) -> lustre@runtime@app:app(any(), nil, MIF).
element(View) ->
    application(
...
-spec simple(
    fun((MIL) -> MIM),
    fun((MIM, MIN) -> MIM),
    fun((MIM) -> lustre@vdom@vnode:element(MIN))
) -> lustre@runtime@app:app(MIL, MIM, MIN).
...
-spec component(
    fun((MJB) -> {MJC, lustre@effect:effect(MJD)}),
    fun((MJC, MJD) -> {MJC, lustre@effect:effect(MJD)}),
    fun((MJC) -> lustre@vdom@vnode:element(MJD)),
    list(lustre@runtime@app:option(MJD))
...
-spec named(
    lustre@runtime@app:app(MJM, MJN, MJO),
    gleam@erlang@process:name(lustre@runtime@server@runtime:message(MJO))
) -> lustre@runtime@app:app(MJM, MJN, MJO).
named(App, Name) ->
    {app,
        {some, Name},
...
-spec do_start(lustre@runtime@app:app(MKG, any(), MKI), binary(), MKG) -> {ok,
        runtime(MKI)} |
    {error, error()}.
do_start(_, _, _) ->
    {error, not_a_browser}.
...
-spec start(lustre@runtime@app:app(MJX, any(), MJZ), binary(), MJX) -> {ok,
        runtime(MJZ)} |
    {error, error()}.
...
-spec start_server_component(lustre@runtime@app:app(MKP, any(), MKR), MKP) -> {ok,
        runtime(MKR)} |
    {error, error()}.
...
-spec supervised(lustre@runtime@app:app(MKY, any(), MLA), MKY) -> gleam@otp@supervision:child_specification(gleam@erlang@process:subject(lustre@runtime@server@runtime:message(MLA))).
...
-spec factory(lustre@runtime@app:app(MLH, any(), MLJ)) -> gleam@otp@factory_supervisor:builder(MLH, gleam@erlang@process:subject(lustre@runtime@server@runtime:message(MLJ))).
factory(App) ->
...
-spec send(runtime(MLY), lustre@runtime@server@runtime:message(MLY)) -> nil.
...
-spec dispatch(MMB) -> lustre@runtime@server@runtime:message(MMB).
dispatch(Message) ->
