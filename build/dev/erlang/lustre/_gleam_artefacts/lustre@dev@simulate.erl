...
-opaque app(NGJ, NGK, NGL) :: {app,
        fun((NGJ) -> {NGK, lustre@effect:effect(NGL)}),
        fun((NGK, NGL) -> {NGK, lustre@effect:effect(NGL)}),
        fun((NGK) -> lustre@vdom@vnode:element(NGL))}.

-opaque simulation(NGM, NGN) :: {simulation,
        fun((NGM, NGN) -> {NGM, lustre@effect:effect(NGN)}),
        fun((NGM) -> lustre@vdom@vnode:element(NGN)),
        list(event(NGN)),
        NGM,
        lustre@vdom@vnode:element(NGN)}.

-type event(NGO) :: {dispatch, NGO} |
...
-spec simple(
    fun((NGP) -> NGQ),
    fun((NGQ, NGR) -> NGQ),
    fun((NGQ) -> lustre@vdom@vnode:element(NGR))
) -> app(NGP, NGQ, NGR).
...
-spec application(
    fun((NGW) -> {NGX, lustre@effect:effect(NGY)}),
    fun((NGX, NGY) -> {NGX, lustre@effect:effect(NGY)}),
    fun((NGX) -> lustre@vdom@vnode:element(NGY))
) -> app(NGW, NGX, NGY).
...
-spec start(app(NHF, NHG, NHH), NHF) -> simulation(NHG, NHH).
start(App, Args) ->
...
-spec message(simulation(NHN, NHO), NHO) -> simulation(NHN, NHO).
...
-spec problem(simulation(NIT, NIU), binary(), binary()) -> simulation(NIT, NIU).
...
-spec click(simulation(NIA, NIB), lustre@dev@query:'query'()) -> simulation(NIA, NIB).
...
-spec input(simulation(NIG, NIH), lustre@dev@query:'query'(), binary()) -> simulation(NIG, NIH).
...
-spec model(simulation(NIZ, any())) -> NIZ.
model(Simulation) ->
    erlang:element(5, Simulation).
...
-spec view(simulation(any(), NJE)) -> lustre@vdom@vnode:element(NJE).
view(Simulation) ->
    erlang:element(6, Simulation).
...
-spec history(simulation(any(), NJJ)) -> list(event(NJJ)).
history(Simulation) ->
