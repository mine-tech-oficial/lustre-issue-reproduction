...
-spec on_attribute_change(binary(), fun((binary()) -> {ok, LST} | {error, nil})) -> lustre@runtime@app:option(LST).
...
-spec on_property_change(binary(), gleam@dynamic@decode:decoder(LSX)) -> lustre@runtime@app:option(LSX).
...
-spec on_context_change(binary(), gleam@dynamic@decode:decoder(LTA)) -> lustre@runtime@app:option(LTA).
...
-spec on_form_autofill(fun((binary()) -> LTF)) -> lustre@runtime@app:option(LTF).
...
-spec on_form_reset(LTH) -> lustre@runtime@app:option(LTH).
...
-spec on_form_restore(fun((binary()) -> LTJ)) -> lustre@runtime@app:option(LTJ).
...
-spec on_form_disabled(fun((boolean()) -> LTL)) -> lustre@runtime@app:option(LTL).
...
-spec on_connect(LTT) -> lustre@runtime@app:option(LTT).
on_connect(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_adopt(LTV) -> lustre@runtime@app:option(LTV).
on_adopt(Message) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_disconnect(LTX) -> lustre@runtime@app:option(LTX).
...
-spec default_slot(
    list(lustre@vdom@vattr:attribute(LTZ)),
    list(lustre@vdom@vnode:element(LTZ))
) -> lustre@vdom@vnode:element(LTZ).
...
-spec named_slot(
    binary(),
    list(lustre@vdom@vattr:attribute(LUF)),
    list(lustre@vdom@vnode:element(LUF))
) -> lustre@vdom@vnode:element(LUF).
...
-spec prerender(
    lustre@runtime@app:app(nil, any(), LVF),
    binary(),
    list(lustre@vdom@vattr:attribute(LVF)),
    list(lustre@vdom@vnode:element(LVF))
) -> lustre@vdom@vnode:element(LVF).
