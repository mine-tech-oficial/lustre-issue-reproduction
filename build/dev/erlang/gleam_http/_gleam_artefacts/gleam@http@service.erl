...
-spec map_response_body(
    fun((QDJ) -> gleam@http@response:response(QDK)),
    fun((QDK) -> QDM)
) -> fun((QDJ) -> gleam@http@response:response(QDM)).
...
-spec prepend_response_header(
    fun((QDO) -> gleam@http@response:response(QDP)),
    binary(),
    binary()
) -> fun((QDO) -> gleam@http@response:response(QDP)).
...
-spec ensure_post(gleam@http@request:request(QDS)) -> {ok,
        gleam@http@request:request(QDS)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(QEB)) -> QED)) -> fun((gleam@http@request:request(QEB)) -> QED).
