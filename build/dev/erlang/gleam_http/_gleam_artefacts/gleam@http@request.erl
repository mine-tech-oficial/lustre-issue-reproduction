...
-spec set_header(request(PPT), binary(), binary()) -> request(PPT).
...
-spec prepend_header(request(PPW), binary(), binary()) -> request(PPW).
...
-spec set_body(request(any()), PQB) -> request(PQB).
set_body(Req, Body) ->
    {request,
...
-spec map(request(PQD), fun((PQD) -> PQF)) -> request(PQF).
...
-spec set_query(request(PQP), list({binary(), binary()})) -> request(PQP).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(PQT), gleam@http:method()) -> request(PQT).
...
-spec set_scheme(request(PRA), gleam@http:scheme()) -> request(PRA).
...
-spec set_host(request(PRD), binary()) -> request(PRD).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(PRG), integer()) -> request(PRG).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(PRJ), binary()) -> request(PRJ).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(PRM), binary(), binary()) -> request(PRM).
...
-spec remove_cookie(request(PRS), binary()) -> request(PRS).
