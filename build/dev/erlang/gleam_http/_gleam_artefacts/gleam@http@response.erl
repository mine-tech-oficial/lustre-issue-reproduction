...
-type response(PZN) :: {response, integer(), list({binary(), binary()}), PZN}.

-file("src/gleam/http/response.gleam", 87).
?DOC(" Set the body of the response, overwriting any existing body.\n").
-spec set_body(response(any()), QAK) -> response(QAK).
...
-spec try_map(response(PZO), fun((PZO) -> {ok, PZQ} | {error, PZR})) -> {ok,
        response(PZQ)} |
    {error, PZR}.
...
-spec set_header(response(QAC), binary(), binary()) -> response(QAC).
...
-spec prepend_header(response(QAF), binary(), binary()) -> response(QAF).
...
-spec map(response(QAM), fun((QAM) -> QAO)) -> response(QAO).
...
-spec expire_cookie(response(QAX), binary(), gleam@http@cookie:attributes()) -> response(QAX).
