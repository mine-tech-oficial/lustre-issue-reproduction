...
-opaque subject(DMR) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(DMR)}.

-type name(DMS) :: any() | {gleam_phantom, DMS}.

-type do_not_leak() :: any().

-type selector(DMT) :: any() | {gleam_phantom, DMT}.
...
-spec 'receive'(subject(DNR), integer()) -> {ok, DNR} | {error, nil}.
...
-spec receive_forever(subject(DNZ)) -> DNZ.
...
-spec selector_receive(selector(DOD), integer()) -> {ok, DOD} | {error, nil}.
...
-spec selector_receive_forever(selector(DOH)) -> DOH.
...
-spec map_selector(selector(DOJ), fun((DOJ) -> DOL)) -> selector(DOL).
map_selector(A, B) ->
...
-spec merge_selector(selector(DON), selector(DON)) -> selector(DON).
merge_selector(A, B) ->
...
-spec select_trapped_exits(selector(DOR), fun((exit_message()) -> DOR)) -> selector(DOR).
...
-spec select_map(selector(DOY), subject(DPA), fun((DPA) -> DOY)) -> selector(DOY).
...
-spec select(selector(DOU), subject(DOU)) -> selector(DOU).
...
-spec deselect(selector(DPD), subject(any())) -> selector(DPD).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> DPI)
) -> selector(DPI).
...
-spec select_other(selector(DPM), fun((gleam@dynamic:dynamic_()) -> DPM)) -> selector(DPM).
...
-spec select_specific_monitor(selector(DPY), monitor(), fun((down()) -> DPY)) -> selector(DPY).
...
-spec select_monitors(selector(DQB), fun((down()) -> DQB)) -> selector(DQB).
...
-spec deselect_specific_monitor(selector(DQE), monitor()) -> selector(DQE).
...
-spec perform_call(
    subject(DQH),
    fun((subject(DQJ)) -> DQH),
    fun((selector(DQJ)) -> {ok, DQJ} | {error, nil})
) -> DQJ.
...
-spec call(subject(DQO), integer(), fun((subject(DQQ)) -> DQO)) -> DQQ.
...
-spec call_forever(subject(DQS), fun((subject(DQU)) -> DQS)) -> DQU.
...
-spec send_after(subject(DRA), integer(), DRA) -> timer().
