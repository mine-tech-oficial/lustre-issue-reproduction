...
-type message(SDY) :: {packet, bitstring()} | {user, SDY}.
...
-type connection(SDZ) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(SDZ))}.

-opaque next(SEA, SEB) :: {continue,
        SEA,
        gleam@option:option(gleam@erlang@process:selector(SEB))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque builder(SEC, SED) :: {builder,
        glisten@socket@options:interface(),
        fun((connection(SED)) -> {SEC,
            gleam@option:option(gleam@erlang@process:selector(SED))}),
        fun((SEC, message(SED), connection(SED)) -> next(SEC, message(SED))),
        gleam@option:option(fun((SEC) -> nil)),
        integer(),
        boolean(),
        boolean(),
        ...
        gleam@option:option(gleam@erlang@process:name(gleam@otp@factory_supervisor:message(glisten@socket:socket(), gleam@erlang@process:subject(glisten@internal@handler:message(SED))))),
...
-spec continue(SFD) -> next(SFD, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten.gleam", 152).
-spec with_selector(next(SFH, SFI), gleam@erlang@process:selector(SFI)) -> next(SFH, SFI).
...
?DOC(false).
-spec convert_next(next(SFW, SFX)) -> glisten@internal@handler:next(SFW, SFX).
convert_next(Next) ->
    case Next of
...
?DOC(false).
-spec map_selector(next(SGC, SGD), fun((SGD) -> SGG)) -> next(SGC, SGG).
...
-spec map_user_selector(gleam@erlang@process:selector(message(SGJ))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(SGJ)).
...
-spec convert_loop(
    fun((SGO, message(SGP), connection(SGP)) -> next(SGO, message(SGP)))
) -> fun((SGO, glisten@internal@handler:loop_message(SGP), glisten@internal@handler:connection(SGP)) -> glisten@internal@handler:next(SGO, glisten@internal@handler:loop_message(SGP))).
convert_loop(Loop) ->
    fun(Data, Msg, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec convert_on_init(
    fun((connection(SGU)) -> {SGW,
        gleam@option:option(gleam@erlang@process:selector(SGU))})
) -> fun((glisten@internal@handler:connection(SGU)) -> {SGW,
    gleam@option:option(gleam@erlang@process:selector(SGU))}).
...
-spec new(
    fun((connection(SHC)) -> {SHE,
        gleam@option:option(gleam@erlang@process:selector(SHC))}),
    fun((SHE, message(SHC), connection(SHC)) -> next(SHE, message(SHC)))
) -> builder(SHE, SHC).
new(On_init, Loop) ->
    {builder,
        loopback,
...
-spec with_close(builder(SHL, SHM), fun((SHL) -> nil)) -> builder(SHL, SHM).
...
-spec with_pool_size(builder(SHR, SHS), integer()) -> builder(SHR, SHS).
...
?DOC(false).
-spec with_http2(builder(SHX, SHY)) -> builder(SHX, SHY).
with_http2(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec bind(builder(SID, SIE), binary()) -> builder(SID, SIE).
...
-spec with_ipv6(builder(SIJ, SIK)) -> builder(SIJ, SIK).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(SIP, SIQ), binary(), binary()) -> builder(SIP, SIQ).
...
-spec with_connection_factory_name(
    builder(SJI, SJJ),
    gleam@erlang@process:name(gleam@otp@factory_supervisor:message(glisten@socket:socket(), gleam@erlang@process:subject(glisten@internal@handler:message(SJJ))))
) -> builder(SJI, SJJ).
