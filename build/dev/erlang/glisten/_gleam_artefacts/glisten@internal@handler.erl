...
-type message(RGK) :: {internal, internal_message()} | {user, RGK}.

-type loop_message(RGL) :: {packet, bitstring()} | {custom, RGL}.

-type loop_state(RGM, RGN) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(RGN)),
        glisten@transport:transport(),
        RGM}.

-type connection(RGO) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(RGO))}.

-type next(RGP, RGQ) :: {continue,
        RGP,
        gleam@option:option(gleam@erlang@process:selector(RGQ))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type handler(RGR, RGS) :: {handler,
        glisten@socket:socket(),
        fun((RGR, loop_message(RGS), connection(RGS)) -> next(RGR, loop_message(RGS))),
        fun((connection(RGS)) -> {RGR,
            gleam@option:option(gleam@erlang@process:selector(RGS))}),
        gleam@option:option(fun((RGR) -> nil)),
        glisten@transport:transport(),
        glisten@socket@options:active_state()}.

-file("src/glisten/internal/handler.gleam", 65).
?DOC(false).
-spec continue(RHF) -> next(RHF, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten/internal/handler.gleam", 69).
?DOC(false).
-spec with_selector(next(RHJ, RHK), gleam@erlang@process:selector(RHK)) -> next(RHJ, RHK).
...
?DOC(false).
-spec start(handler(any(), RHZ)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(message(RHZ)))} |
