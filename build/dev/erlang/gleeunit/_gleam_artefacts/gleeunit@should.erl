...
-spec be_ok({ok, AJRE} | {error, any()}) -> AJRE.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, AJRJ}) -> AJRJ.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(AJRM)) -> AJRM.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
