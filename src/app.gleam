    ...
    html([attribute.lang("en")], [
      ...
      html.body(
        ...
        [
          ...
          html.script(
            [],
            "document.querySelector('lustre-server-component').addEventListener('ready', (event) => {console.log(\"hi!\")});",
          ),
        ],
      ),
    ])
...
type CounterSocket {
  CounterSocket(
    component: lustre.Runtime(component.Message),
    self: Subject(server_component.ClientMessage(component.Message)),
  )
}

type CounterSocketMessage =
  server_component.ClientMessage(component.Message)
